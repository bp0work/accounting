...
<script lang="ts">
  ...
  let director = $state<Array<Record<string, unknown>>>([]);
  let r = $state({
    property_address: '',
    monthly_rent_sgd: '',
  ...
  let d = $state({
    director_name: '',
    director_email: '',
    authorised_expense_types: 'home office,meals',
    monthly_limit_sgd: '',
    effective_date: '',
  });
  ...
  async function addRental() {
    await createRentalAgreement({
      ...r,
      monthly_rent_sgd: Number(r.monthly_rent_sgd),
      business_use_percent: Number(r.business_use_percent),
    });
    rental = await listRentalAgreements();
  }
...
<h1>Agreements</h1>
<div class="tabs">
  <button type="button" class:active={tab === 'rental'} onclick={() => (tab = 'rental')}>Rental</button>
  <button type="button" class:active={tab === 'director'} onclick={() => (tab = 'director')}>
    Director expense
  </button>
</div>
{#if tab === 'rental'}
  <div class="card">
    ...
    <button type="button" onclick={addRental}>Add rental</button>
  </div>
  <ul>
    {#each rental as x}
...
{:else}
  <div class="card">
    ...
    <button type="button" onclick={addDirector}>Add agreement</button>
  </div>
  <ul>
    {#each director as x}
      <li>{x.director_name} — {x.director_email}</li>
    {/each}
  </ul>
{/if}
<style>
  ...
  .card { display: flex; flex-wrap: wrap; gap: 0.5rem; margin-bottom: 1rem; }
