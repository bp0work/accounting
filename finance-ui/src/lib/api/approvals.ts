...
export function bindingQueueForRole(roleName: string | null | undefined): BindingQueue | undefined {
  ...
  if (r === 'accounts_manager' || r === 'finance_officer' || r === 'finance_manager') return 'acc';
...
export type JournalLineAccountUpdate = {
  line_number: number;
  account_id: string;
};
...
export type ApprovePayload = {
  note: string;
  line_account_updates?: JournalLineAccountUpdate[];
};
...
export function approve(id: string, payload: string | ApprovePayload) {
  const body =
    typeof payload === 'string'
      ...
      : {
          note: payload.note,
          ...(payload.line_account_updates?.length
            ? { line_account_updates: payload.line_account_updates }
            : {}),
        };
