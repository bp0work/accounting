"""Expense Worker service — port 8014."""
...
from workers.expense.worker import ExpenseQueueConsumer

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("expense-worker")

settings = get_settings()
app = FastAPI(title="Expense Worker", version=settings.version)
...
async def _consumer_loop() -> None:
    ...
    redis = get_redis()
    consumer = ExpenseQueueConsumer(redis)
    while True:
        try:
            ...
            logger.exception("Expense worker poll error")
            await asyncio.sleep(1)


@app.on_event("startup")
async def startup() -> None:
    if settings.expense_worker_enabled:
...
async def process_once() -> dict:
    ...
    factory = get_session_factory()
    async with factory() as session:
        worked = await ExpenseQueueConsumer(redis).poll_once(session)
