...
| File | Purpose |
|------|---------|
| `api-routes.yml` | Routes API path prefixes on `finance.mmlogistix.bp0.work` to `http://fastapi:8000`. Single-line `rule`; router `finance-api` references service **`finance-api`** (same key under `http.services`). |
