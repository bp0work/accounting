...
|----------------|------|------------|---------|
| **0.12.9-mail-pdf-sanitize** | 2026-05-20 | *(pending)* | Strip NUL bytes from PDF `extracted_text` before DB insert (`sanitize_extracted_text` in ingest + email_context). Fixes PostgreSQL `CharacterNotInRepertoireError`. |
