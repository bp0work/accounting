...
|----------------|------|------------|---------|
| **0.13.20-docx-po-travel-controls** | 2026-05-24 | `a2b3c7f` | **DOCX:** `python-docx` in `attachment_text.py` + DOCX MIME on executive mailboxes (migration `048`). **AP:** `due_date` critical field; PO gate — post only on `po_validation` `exact`/`partial`; `not_found`/`mismatch` → manager escalation; block in `workflow_metadata.po_validation`. **Expense:** `travel_requests` table + travel-category claim matching; missing request → manager escalation. |
