...
|----------------|------|------------|---------|
| **0.13.10-ap-client-vendor-column-fix** (finance-ui) | 2026-05-20 | — | Approvals + case detail **Client / Vendor**: AP (`ap_invoice`, `ap_po_validation`, `ap_payment_proposal`) → `client_vendor_name`; AR → `counterparty_name` only (`clientVendorColumnValue` in `case-labels.ts`). Requires API `client_vendor_name` (`0.13.21`). |
