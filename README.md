...
**Release version:** `0.15.03a-reversal-message-type` (FastAPI and Finance UI — see `accfin/app/core/config.py` → `Settings.version`, `finance-ui/package.json`, and `/api/health`). Client Admin: `0.14.69-login-portal-links`. Operator release notes and deploy matrix: local `platform_dox/` (not in git).
